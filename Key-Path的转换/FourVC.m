...
- (void) convertByDict:(NSDictionary *) dict{
    PersonFour *person = [PersonFour yy_modelWithDictionary:dict];
    NSLog(@"name=%@,age=%@,weight=%@,bookName=%@",person.name,person.age,person.weight,person.bookName);
}

//获取base.json中的json字符串后转字典
- (NSDictionary *) getDict{
    
    NSString *path = [[NSBundle mainBundle] pathForResource:@"four" ofType:@"json"];
